    ...
    while not done:
        ...
        if collect_traces:
            ...
            traces.append(dict(
                ...
                time_norm=float(obs.get('time_norm', 0.0)),
                current_time=float(obs.get('current_time', 0.0)),
                battery_abs=float(obs['battery_norm'] * inst['battery_cap']),
                cargo_abs=float(obs['cargo_norm'] * inst['cargo_cap']),
                arrival_time=float(env.time + env.D[obs['current_node'], action]),
                ready_time=float(inst.get('ready_times', np.zeros(env.n))[action]),
                due_time=float(inst.get('due_times', np.full(env.n, inst.get('time_horizon', 1.0)))[action]),
                service_time=float(inst.get('service_times', np.zeros(env.n))[action]),
