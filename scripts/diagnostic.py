...
def main():
    ...
    parser.add_argument("--explain", action="store_true",
                        help="Generate LLM explanation of the diagnostic via Groq "
                             "(requires GROQ_API_KEY and pip install 'rl4evrp[llm]')")
    parser.add_argument("--explain-output", default=None,
                        help="Path to save the LLM explanation. Defaults to <output>.txt")
    ...
    if args.explain:
        try:
            ...
            explain_path = args.explain_output or args.output.replace(".pdf", "_explanation.txt")
            print("\nGenerating LLM explanation...")
            explanation = explainer.explain_diagnostic(diag, scores, drift, explain_path)
            print(f"\n{'='*60}")
            print("LLM DIAGNOSTIC EXPLANATION")
            print(f"{'='*60}")
            print(explanation)
            print(f"\nExplanation saved to {explain_path}")
